 ...
 */
export function initializeCrossfadingPlayer() {
 ...
 * @returns {number[]} An array of silence start and end times in seconds.
 */
export function detectSilence(audioData: Float32Array, threshold: number, minSilenceDuration: number): number[] {
    const silenceTimes: number[] = [];
    let silenceStart = -1;
    let silenceEnd = -1;
    ...
    for (let i = 0; i < audioData.length; i++) {
        if (Math.abs(audioData[i]) < threshold) {
            ...
            if (silenceDuration >= minSilenceDuration) {
                silenceEnd = i;
                silenceTimes.push(silenceStart / audioData.length, silenceEnd / audioData.length);
            }
            silenceStart = -1;
            silenceDuration = 0;
        }
